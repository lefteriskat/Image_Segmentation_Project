...
def compute_iou(predicted_mask, ground_truth):
    ...
    iou_value = iou(predicted_mask.cpu(), ground_truth.cpu())
    return iou_value

def compute_dice(predicted_mask, ground_truth):
    dice = Dice()
    dice_value = dice(predicted_mask.cpu(), ground_truth.cpu())
    return dice_value

def compute_specificity(predicted_mask, ground_truth):
    metric = BinarySpecificity(threshold = 0.5)
    specificity = metric(predicted_mask.cpu(), ground_truth.cpu())
    return specificity

def compute_sensitivity(predicted_mask, ground_truth):
    sensitivity = torchmetrics.functional.classification.binary_recall(predicted_mask.cpu(), ground_truth.cpu())
    return sensitivity